...
export default function QuickSignInCheck({
  limit = 10,
  showFailedOnly = false,
}: QuickSignInCheckProps) {
  ...
  return (
    <div className="bg-white rounded-lg shadow">
      ...
      <div className="divide-y divide-gray-200">
        {records.length === 0 ? (
          <div className="px-4 py-3 text-sm text-gray-500 text-center">No records found</div>
        ) : (
          records.map((record) => (
            <div key={record.id} className="px-4 py-3">
              <div className="flex items-center justify-between">
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-gray-900 truncate">{record.email}</p>
              ...
              {record.device_info &&
                typeof record.device_info === "object" &&
                "browser" in record.device_info &&
                "os" in record.device_info && (
